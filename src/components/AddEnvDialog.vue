...
async function add() {
  ...
  const name = input.value.trim(); // 纯空格视同空，避免把空白当分支名传给 gws
  if (!name) return;
  err.value = "";
...
<template>
  <div class="mask" @click.self="!submitting && emit('close')">
    <div class="dialog">
      <h3>添加环境</h3>
      <label>环境分支名 <input v-model="input" autocapitalize="off" spellcheck="false" placeholder="如 pre、dev1（须全小写）" /></label>
      <p class="muted">在各仓库检出同名远程分支为 worktree；远程无该分支时报错不建</p>
