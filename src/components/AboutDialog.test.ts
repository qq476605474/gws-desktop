...
describe("AboutDialog 一键更新", () => {
  it("update：args 于 hub.path，exit 前 version 未被重查（waitDone 锁时序）；在途禁关闭/禁点 mask；完成后刷新版本并显示已是最新", async () => {
    ...
    // 负向断言：exit 事件未发出、更新未到终态前，不得提前重查版本（exec 返回时 update 仍在跑）
    expect(mocks.runGws).toHaveBeenCalledTimes(1);
  ...
  it("双击守卫：首击在途（updating）时同步第二击不重复 exec", async () => {
    ...
    clickButton("更新到 0.5.0");
    clickButton("更新到 0.5.0"); // happy-dom 的 click() 不受 disabled 限制，第二击只能靠入口守卫拦截
  ...
  it("isRunning（外部命令在跑）：检查/更新按钮禁用且点击不发 exec，关闭不受影响", async () => {
    const closed = vi.fn();
    mountDialog(closed);
    await vi.waitFor(() => expect(el!.textContent).toContain("0.4.2"));
    clickButton("检查更新");
    ...
    // disabled 拦不住 happy-dom 的 click()：入口守卫须兜底（update 的 isRunning 分支）
    clickButton("更新到 0.5.0");
